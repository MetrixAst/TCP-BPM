...
| 12 | Техническая документация | Средне |

Подробности: `SPRINT_REPORT_05-18.md` → раздел «Правки».

### 3.1. Prod-ready и интеграции (01.06.2026)

**Цель:** к prod вывести рабочие блоки — HR, документооборот, заявки арендаторов, заявки, задачи между сотрудниками.

| # | Правка | Приоритет | Трек |
|---|--------|-----------|------|
| 13 | **ONLYOFFICE** — онлайн-редактирование документов на сервере | P0 | BE + DevOps |
| 14 | **Документооборот** — редактирование через ONLYOFFICE (callback save) | P0 | BE + FE |
| 15 | ONLYOFFICE: JWT, права на документ при открытии редактора | P0 | BE |
| 16 | **Персональное рабочее время** у каждого сотрудника (график/смены) | P1 | BE + FE |
| 17 | Учёт персонального графика в посещаемости и опозданиях | P1 | BE |
| 18 | **Задачи** — полировка UI, всё функционирует E2E | P0 | FE |
| 19 | Дизайн по **видео-референсу** MetriX | P1 | FE |
| 20 | **Не localStorage** — фильтры, черновики, настройки UI → **БД** / session | P1 | BE + FE |
| 21 | **Заявки арендаторов** — prod-ready (CRUD, статусы, уведомления) | P0 | BE + FE |
| 22 | **Заявки** (внутренние / общий модуль) — prod-ready | P0 | BE + FE |

#### ONLYOFFICE (документооборот)

- Document Server (Docker) рядом с Django или отдельный хост.
- Backend: генерация конфига редактора, `callback` при сохранении, версионирование файла.
- Frontend: embed ONLYOFFICE в карточке документа (кадровые + документооборот).
- Файлы хранятся на сервере (media/S3), не уходят в браузер для редактирования.

#### Персональное рабочее время

- У каждого `Employee` — свой график (дни недели, начало/конец, тип смены).
- Связь с `WorkCalendar` (праздники РК) и `AttendanceRecord` (опоздания по **его** графику, не общему).

#### Prod-ready чеклист

| Модуль | Критерий готовности |
|--------|---------------------|
| HR | Сотрудники, оргструктура, отпуска, календарь, посещаемость |
| Документооборот | Загрузка, версии, ONLYOFFICE, права, согласование |
| Заявки арендаторов | Полный цикл заявки в prod |
| Заявки | Внутренние заявки сотрудников в prod |
| Задачи | Создание → маршрут → канбан → история между сотрудниками |

#### Хранение данных

- **Запрещено** для бизнес-состояния: `localStorage`, `sessionStorage` (фильтры финансов, очередь 1С, черновики задач, настройки таблиц).
- **Разрешено:** Django `session`, модели `UserPreference` / `UserUIState`, POST save на backend.

Подробности: `SPRINT_REPORT_05-18.md` → «Правки и prod-scope — 1 июня 2026».
