...
class Calendar {
  ...
  constructor(options = {}) {
    ...
    Helpers.FetchJSON(Helpers.UrlFix('/finances/calendar/action/json'), (data) => {
      this.events = data;
      this._addColors();
  ...
  _eventClick(info) {
    ...
    document.getElementById('eventText').value = event.extendedProps.text;
    document.getElementById('eventCategory').value = event.extendedProps.category;
    document.getElementById('modalTitle').innerHTML = 'Редактирование';
  ...
  _updateEventConfirm() {
    ...
    currentEvent.text !== document.getElementById('eventText').value && currentEvent.setProp('text', document.getElementById('eventText').value);
    ...
    var date = new Date(jQuery('#eventStartDate').datepicker('getDate'));
    date = moment(date).format('YYYY-MM-DD');

    const obj = currentEvent.toPlainObject();

    this._sendToServer('edit', {
      title: obj.title,
      date: date,
      category: obj.extendedProps.category,
      text: obj.extendedProps.text,
    });
  }
  ...
  _addNewEvent() {
    ...
    this.currentEventId = null;
    this._enableAdd();
    document.getElementById('modalTitle').innerHTML = 'Добавить';
    this.newEventModal.show();
  }
  ...
  _sendToServer(action, data){
    const obj = this;
    const csrf = $('#calendar').attr('data-csrf');
    data.csrfmiddlewaretoken = csrf;
    
    //set category
    if(data.category !== undefined){
      data.category = obj._toCategorySlug(data.category);
    }

    $.ajax({
      url: `/finances/calendar/action/${action}/`,
      method: 'post',
      data: data,
      success: function(res) {
        ...
        switch (action) {
          case 'create':
            obj.calendar.addEvent({
              title: res.title,
              start: res.date,
              end: res.date,
              id: res.id,
              category: res.category,
              color: obj._getColorByCategory(res.category),
              text: res.text,
            });
            break;
          default:
  ...
  _deleteConfirmClick() {
    ...
    this._sendToServer('delete', {'id': currentEvent.id})
  ...
  _getColorMap() {
    return [
      {color: Globals.success, category: 'Доход'},
      {color: Globals.danger, category: 'Расход'},
    ];
  }
